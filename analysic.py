...
for comm in commodity_list:
    for year in range(1970,2024,10):
        ...
        down_res = requests.get(url.format(method=method,dateFrom=dateFrom,dateTo=dateTo,commodity=comm,dataType=dataType))
        if 'No results' not in str(down_res.content):
            with open(f'{comm}_{dateFrom}.xlsx','wb') as f:
                f.write(down_res.content)
            time.sleep(10)
        else:
            print('No results,so can\'t create the file!')